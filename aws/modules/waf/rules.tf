locals {
  byte_match_statement_rules = local.count && var.byte_match_statement_rules != null ? {
    for rule in flatten(var.byte_match_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-byte-match-%d", rule.priority),
      rule.action,
    ) => rule
  } : {}

  geo_allowlist_statement_rules = local.count && var.geo_allowlist_statement_rules != null ? {
    for rule in flatten(var.geo_allowlist_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-geo-allowlist-%d", rule.priority),
      "block",
    ) => rule
  } : {}

  geo_match_statement_rules = local.count && var.geo_match_statement_rules != null ? {
    for rule in flatten(var.geo_match_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-geo-match-%d", rule.priority),
      rule.action,
    ) => rule
  } : {}

  ip_set_reference_statement_rules = local.count && var.ip_set_reference_statement_rules != null ? {
    for indx, rule in flatten(var.ip_set_reference_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-ip-set-reference-%d", rule.priority),
      rule.action,
    ) => rule
  } : {}

  managed_rule_group_statement_rules = local.count && var.managed_rule_group_statement_rules != null ? {
    for rule in flatten(var.managed_rule_group_statement_rules) :
    lookup(rule, "name", null) != null ? rule.name : format("%s-managed-rule-group-%d", rule.priority) => rule
  } : {}

  rate_based_statement_rules = local.count && var.rate_based_statement_rules != null ? {
    for rule in flatten(var.rate_based_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-rate-based-%d", rule.priority),
      rule.action,
    ) => rule
  } : {}

  rule_group_reference_statement_rules = local.count && var.rule_group_reference_statement_rules != null ? {
    for rule in flatten(var.rule_group_reference_statement_rules) :
    lookup(rule, "name", null) != null ? rule.name : format("%s-rule-group-reference-%d", rule.priority) => rule
  } : {}

  regex_pattern_set_reference_statement_rules = local.count && var.regex_pattern_set_reference_statement_rules != null ? {
    for rule in flatten(var.regex_pattern_set_reference_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-regex-pattern-set-reference-%d", rule.priority),
      rule.action,
    ) => rule
  } : {}

  regex_match_statement_rules = local.count && var.regex_match_statement_rules != null ? {
    for rule in flatten(var.regex_match_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-regex-match-statement-%d", rule.priority),
      rule.action,
    ) => rule
  } : {}

  size_constraint_statement_rules = local.count && var.size_constraint_statement_rules != null ? {
    for rule in flatten(var.size_constraint_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-size-constraint-%d", rule.priority),
      rule.action,
    ) => rule
  } : {}

  sqli_match_statement_rules = local.count && var.sqli_match_statement_rules != null ? {
    for rule in flatten(var.sqli_match_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-sqli-match-%d", rule.priority),
      rule.action,
    ) => rule
  } : {}

  xss_match_statement_rules = local.count && var.xss_match_statement_rules != null ? {
    for rule in flatten(var.xss_match_statement_rules) :
    format("%s-%s",
      lookup(rule, "name", null) != null ? rule.name : format("%s-xss-match-%d", rule.priority),
      rule.action,
    ) => rule
  } : {}
}
