...
resource "azurerm_windows_virtual_machine_scale_set" "winsrv_vmss" {
  ...
  name                   = format("%s%s", lower(replace(var.vmscaleset_name, "/[[:^alnum:]]/", "")), count.index + 1)
  ...
  tags                   = merge({ "ResourceName" = format("%s%s", lower(replace(var.vmscaleset_name, "/[[:^alnum:]]/", "")), count.index + 1) }, var.tags, )
  source_image_id        = var.source_image_id != null ? var.source_image_id : null
  upgrade_mode           = var.os_upgrade_mode
  #health_probe_id        = var.enable_load_balancer ? azurerm_lb_probe.lbp[0].id : null
  provision_vm_agent     = true
